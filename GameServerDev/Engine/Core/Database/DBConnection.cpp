...
std::unique_ptr<DBStatement> DBConnection::CreateStatement()
{
	return std::make_unique<DBStatement>(this);
}

std::unique_ptr<DBTransaction> DBConnection::StartTransaction()
{
	BeginTransaction();
	return std::make_unique<DBTransaction>(this);
}
